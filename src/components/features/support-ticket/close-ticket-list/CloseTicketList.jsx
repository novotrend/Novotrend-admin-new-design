...
import { useCloseSupportTicketListQuery } from "@/services/supportticket/supportticket.query";
import QuestionDocModal from "../open-ticket-list/QuestionDocModal";
...
const getRecords = data => {
  ...
  if (Array.isArray(response?.close_tickets)) return response.close_tickets;
  if (Array.isArray(response?.closed_tickets)) return response.closed_tickets;
...
export default function CloseTicketList() {
  ...
  const { data, isLoading, error } = useCloseSupportTicketListQuery({
    limit,
    offset,
  ...
  return (
    <>
            ...
            <DataTable headers={tableHeaders}>
              ...
              {error && (
                <TableRow>
                  ...
                  >
                    {error?.message || "Failed to load close support tickets."}
                  </TableCell>
                </TableRow>
              )}
              ...
              {!isLoading && !error && tickets.length === 0 && (
                <TableRow>
                  <TableCell
                    ...
                    className="px-6 py-8 text-center text-sm text-muted-foreground"
                  >
                    No close support tickets found.
                  </TableCell>
                </TableRow>
              )}
              ...
              {!isLoading &&
                ...
                tickets.map((item, index) => (
                  ...
                  >
                    ...
                    <TableCell className="px-6 py-5">
                      <Link href={`/support-ticket/support-details?ticket_id=${item.id}&status=closed`}>
      ...
      <QuestionDocModal open={open} onOpenChange={setOpen} selectedTicket={selectedTicket} />
    </>
  );
}
