...
import { useWithdrawalAcceptListQuery } from "@/services/withdrawalrequest/withdrawal.query";
...
export default function AcceptedList() {
  ...
  const { data, isLoading, isError } = useWithdrawalAcceptListQuery({
    limit,
    offset,
    search: debouncedSearch,
  });
  const acceptedList = data?.response?.withdrawal_requests || [];
  ...
  return (
    <div className="overflow-hidden rounded-3xl border border-border bg-background">
      <div className="overflow-x-auto">
        <TableWrapper
          title="Accepted Withdrawal List"
          description="View all accepted withdrawal requests"
          actions={
            <>
